...
test('a 30k-member server returns the full filtered count with a bounded preview and no export', async () => {
  ...
  const reader = new DiscordReader({
    get: async (route, options) => {
      ...
      if (route !== Routes.guildMembers(guildId)) throw new Error(`Unexpected route: ${route}`);
      ...
      return Array.from({ length: Math.min(1_000, 30_001 - offset) }, (_, index) => {
  ...
  const output = JSON.parse(await tool.run({
    data: { allOf: ['Verified'], anyOf: [], noneOf: ['Muted'], memberType: 'humans' },
