...
`DiscordAnalyst` uses Anthropic Claude Haiku 4.5 by default and the Discord REST
...
The default model is `anthropic/claude-haiku-4-5`. Set `DISCORD_ANALYST_MODEL`
in `discord/.env`, or prefix a command to select a model for that invocation.
Flue accepts `provider/model` IDs; OpenRouter IDs include the upstream provider
and model after `openrouter/`.
...
```sh
DISCORD_ANALYST_MODEL=anthropic/claude-haiku-4-5 pnpm ask:continue --message "Summarize the server."
DISCORD_ANALYST_MODEL=openai/gpt-5.5 pnpm ask:continue --message "Check the active threads."
DISCORD_ANALYST_MODEL=openrouter/moonshotai/kimi-k2.6 pnpm ask:continue --message "Count members by role."
```
