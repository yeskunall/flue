...
const ROLE_RISK_PERMISSIONS: ReadonlyArray<{
  ...
  { flag: PermissionFlagsBits.Administrator, name: 'Administrator', severity: 'high' },
  { flag: PermissionFlagsBits.ManageGuild, name: 'ManageGuild', severity: 'high' },
  { flag: PermissionFlagsBits.ManageRoles, name: 'ManageRoles', severity: 'high' },
  { flag: PermissionFlagsBits.ManageChannels, name: 'ManageChannels', severity: 'high' },
  { flag: PermissionFlagsBits.ManageWebhooks, name: 'ManageWebhooks', severity: 'high' },
  { flag: PermissionFlagsBits.BanMembers, name: 'BanMembers', severity: 'high' },
  { flag: PermissionFlagsBits.KickMembers, name: 'KickMembers', severity: 'high' },
  { flag: PermissionFlagsBits.ModerateMembers, name: 'ModerateMembers', severity: 'high' },
  { flag: PermissionFlagsBits.ManageMessages, name: 'ManageMessages', severity: 'high' },
  { flag: PermissionFlagsBits.ManageThreads, name: 'ManageThreads', severity: 'high' },
  { flag: PermissionFlagsBits.ManageEvents, name: 'ManageEvents', severity: 'high' },
  { flag: PermissionFlagsBits.MentionEveryone, name: 'MentionEveryone', severity: 'medium' },
  { flag: PermissionFlagsBits.ViewAuditLog, name: 'ViewAuditLog', severity: 'medium' },
];
...
export function calculateInactiveChannels(
  ...
  for (const observation of observations) {
    ...
    const inactiveForDays = Math.floor(
      (observedAt - Date.parse(observation.latestVisibleMessageAt)) / MILLISECONDS_PER_DAY,
    );
...
export function assessPermissionRisks(input: {
  ...
  for (const role of input.roles) {
    ...
    for (const riskPermission of ROLE_RISK_PERMISSIONS) {
      ...
      risks.push({
        severity: isEveryone && riskPermission.severity === 'high' ? 'high' : riskPermission.severity,
        subjectType: 'role',
        subjectId: role.id,
  ...
  return risks.toSorted((left, right) => severityOrder[left.severity] - severityOrder[right.severity]);
}
